...
namespace PayRexApplication.Services
{
    public interface ISubscriptionService
    {
        ...
        Task<(bool Success, string Error)> ScheduleDowngradeAsync(int companyId, int newPlanId);
    }
    ...
    public class SubscriptionService : ISubscriptionService
    {
        ...
        public async Task<SubscriptionDto?> GetCompanySubscriptionAsync(int companyId)
        {
            ...
            return new SubscriptionDto
            {
                ...
                PendingDowngradePlanId = sub.PendingDowngradePlanId,
                PendingDowngradePlanName = sub.PendingDowngradePlan?.Name
            };
        }
        ...
        public async Task<bool> ProcessTestPaymentAsync(int paymentId, int companyId, string paymentMethod)
        {
            ...
            var invoiceAmount = invoice?.Amount ?? payment.Amount ?? 0m;
            var invoiceRef = invoice?.InvoiceNumber ?? payment.PaymentId.ToString();
            var vatAmount = invoiceAmount * 0.12m;
        ...
        public async Task<(bool Success, string Error)> ScheduleDowngradeAsync(int companyId, int newPlanId)
        {
            var sub = await _db.Subscriptions
                .Include(s => s.SubscriptionPlan)
                .Where(s => s.CompanyId == companyId)
            ...
            if (sub.Status == SubscriptionStatus.Expired || sub.Status == SubscriptionStatus.Cancelled)
                return (false, "Current subscription is not active.");

            var newPlan = await _db.SubscriptionPlans.FindAsync(newPlanId);
            if (newPlan == null) return (false, "Plan not found.");
            if (newPlan.Price >= sub.SubscriptionPlan.Price)
            ...
            var company = await _db.Companies.FindAsync(companyId);
            var companyName = company?.CompanyName ?? "A company";

            _db.SystemNotifications.Add(new SystemNotification
            {
                Title = "Plan Downgrade Scheduled",
                Message = $"Your plan will be downgraded to {newPlan.Name} on {sub.EndDate:MMM dd, yyyy} when your current plan expires.",
                Type = "warning",
            ...
            await _audit.LogAsync(null, companyId, "DowngradeScheduled", "Subscription",
                sub.SubscriptionId.ToString(), null, $"Downgrade to {newPlan.Name} on {sub.EndDate:MMM dd, yyyy}");
